  ...
  <div class="page-list">
    ...
    <div class="table-toolbar">
      ...
      <el-tooltip effect="dark" content="刷新表格" placement="top">
        <svg-icon name="reload" @click.stop="getPagePage" class="table-toolbar-item"/>
      </el-tooltip>
      <el-button
        type="primary"
        size="mini"
            ...
            >
              <div style="text-align: right; margin: 0">
                <el-button
                  type="text"
                  size="mini"
                  @click="updatePageStatus(row, blogConst.POST_STATUS_PUBLISH)"
                >
                  已发布
                </el-button>
                <el-button
                  type="text"
                  size="mini"
                  @click="updatePageStatus(row, blogConst.POST_STATUS_DRAFT)"
                >
                 草稿
                </el-button>
                <el-button
                  type="text"
                  size="mini"
                  @click="updatePageStatus(row, blogConst.POST_STATUS_TRASH)"
                >
                  垃圾箱
                </el-button>
              </div>
...
export default {
  ...
  methods: {
    getPagePage() {
      ...
      pageApi.getPagePage({ ...this.queryParam, ...this.pagination })
        .then(res => {
          const { data } = res;
    ...
    updatePageStatus(row, status) {
      pageApi.updatePageStatus(row.id, status)
        .then(() => {
    ...
    deleteBatch() {
      const selectRow = this.$refs.pageTable.selection;
      ...
      if (deleteIds.length > 0) {
        this.tableLoading = true;
        pageApi.deletePageBatch(deleteIds)
          .then(() => {
